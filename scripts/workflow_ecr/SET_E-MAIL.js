...
var MAINENGINEER = $.context.MAINENGINEER;
var SECONDENGINEER = $.context.SECONDENGINEER;
for (var i = 0; i < ApprovalUser.length; i++) {
	for (var m = 0; m < UserMail.length; m++) {
		if (ApprovalUser[i].APPROVALACCOUNT === UserMail[m].ACCOUNT & UserMail[m].EMAIL !== "NULL") {
			mail1.push(UserMail[m].EMAIL);
		}
		if (UserMail[m].ACCOUNT === MAINENGINEER) {
			mail1.push(UserMail[m].EMAIL);
		}
		for (var n = 0; n < SECONDENGINEER.length; n++) {
			if (UserMail[m].ACCOUNT === SECONDENGINEER[n].key) {
				mail1.push(UserMail[m].EMAIL);
			}
		}
	}
